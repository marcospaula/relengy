...
def test_heavy_censoring_flips_to_mle():
    ...
    assert r.method == "MLE"
    assert "censura pesada" in r.reason
...
def test_small_sample_boundary():
    ...
    assert "pequena" in recommend_method(20, 0).reason
    assert "pequena" not in recommend_method(21, 0).reason
...
def test_diagnosis_exposes_censoring_fraction_and_recommendation():
    ...
    assert "METODO RECOMENDADO" in d.report()

def test_report_changes_when_interval_data_declared():
    d = fit_diagnostic([30, 49, 82, 90, 96], [10, 45, 100])
    assert "RRX" in d.report()
    assert "MLE" in d.report(interval_data=True).split("METODO RECOMENDADO")[1]
